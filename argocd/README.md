...
### Contenu
- `bootstrap/app-of-apps-dev.yaml`: Application Argo CD "app-of-apps" pointant vers `argocd/apps` sur la branche `develop`.
- `projects/platform-dev.yaml`: Projet `platform` autorisant les sources Helm/repos nécessaires.
...
```bash
kubectl apply -f projects/platform-dev.yaml
kubectl apply -f bootstrap/app-of-apps-dev.yaml
```
